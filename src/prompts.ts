  ...
  registrar.registerPrompt<ProvisionLockedBucketArgs>(
    ...
    (args) => {
      return promptResult("Guided B2 Object Lock bucket provisioning workflow.", [
        ...
        "4. Call `b2_update_bucket` with `fileLockEnabled: true` and the requested `defaultRetention`.",
  ...
  registrar.registerPrompt<RotateApplicationKeyArgs>(
    ...
    (args) =>
      promptResult("Guided B2 application-key rotation workflow.", [
        ...
        "3. Show the replacement key request and ask the user to approve creating a durable credential before calling `b2_create_key` with a fresh idempotency key.",
        "4. Verify the created key metadata from the tool response. Do not print or persist the key secret except through the configured durable secret sink behavior.",
        "5. Ask the user to confirm cutover is complete before calling `b2_delete_key` for the old key.",
        "",
        DESTRUCTIVE_GATE_NOTE,
      ]),
  );
