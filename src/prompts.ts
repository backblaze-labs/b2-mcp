    ...
    ({ bucketName, riskContext }) => {
      return workflowPrompt(
        ...
        promptText(
          ...
          "1. If bucketName is provided, call b2_list_buckets filtered by the exact bucketName. If bucketName is omitted, call b2_list_buckets with bucketTypes ['allPublic'] and limit 1000 to directly enumerate public buckets.",
          "2. Treat any truncated response as incomplete. State that the audit cannot prove a clean result beyond the returned slice, and ask for a narrower bucketName/bucketTypes filter or an operator-side export before reporting account-wide closure.",
          "3. Classify bucketType allPublic as public exposure and allPrivate as private. Note CORS, lifecycle, Object Lock, and replication fields when present, but do not infer object-level access from bucket type alone.",
          "4. Summarize public buckets, likely data exposure, operational owners to verify, and low-risk next steps.",
          "5. For remediation, propose exact b2_update_bucket changes such as changing allPublic to allPrivate, but wait for user approval before changing bucket settings.",
          "6. Re-list targeted buckets after any approved change and report residual exposure.",
        ),
      );
    },
  );
    ...
    ({ bucketName, costGoal, prefix, retentionRequirement }) => {
      return workflowPrompt(
        ...
        promptText(
          ...
          "3. Draft a complete replacement lifecycle configuration assembled from the current rules plus the proposed optimization rule; do not submit only the new rule.",
          "4. Use only B2-supported AbortIncompleteMultipartUpload, Expiration, or NoncurrentVersionExpiration rules, and do not propose unsupported storage-class transition rules.",
          "5. If the current lifecycle configuration cannot be reconstructed exactly from tool results, stay plan-only and hand off the complete intended replacement for operator review instead of calling an update tool.",
          "6. Before calling s3_put_bucket_lifecycle or b2_update_bucket, explain every preserved rule and changed rule, including exact rule ID, prefix, age threshold, affected object class, estimated impact, and rollback limits.",
          "7. If a rule schedules deletion, expiration, or clears lifecycle configuration, stop for the destructive gate or MCP elicitation before applying it.",
          "8. After any approved update, re-check the bucket rules and report the final lifecycle configuration.",
        ),
      );
    },
  );
