...
export function registerS3ObjectTools(server: McpServer, s3: S3Client): void {
  ...
  server.tool(
    ...
    "Upload an object to a B2 bucket via the S3-compatible API. Provide either base64-encoded content or a local file path.",
    {
      ...
      storageClass: z.string().optional().describe("Storage class, e.g. STANDARD (B2 ignores this but accepts it for S3 compatibility)."),
    },
    async (args) => {
      try {
        ...
        const contentLength = args.filePath
          ? fs.statSync(args.filePath).size
          : undefined;
  ...
  server.tool(
    ...
    "Delete multiple objects from a B2 bucket in a single request (up to 1000 objects).",
    {
      ...
      })).max(1000).describe("Array of objects to delete."),
      quiet: z.boolean().optional().default(true)
  ...
  server.tool(
    ...
    "Copy an object within B2 or between B2 buckets via the S3-compatible API.",
    {
      ...
      sourceVersionId: z.string().optional().describe("Copy a specific version of the source object."),
      ...
      metadata: z.record(z.string(), z.string()).optional().describe("New metadata (only used with REPLACE)."),
    ...
    async (args) => {
      try {
        ...
        return toolSuccess(`Copied '${args.sourceKey}' to '${args.destinationBucket}/${args.destinationKey}'.`);
      } catch (err) { return toolError(err); }
    }
  );
  ...
  server.tool(
    ...
    {
      bucket: z.string().describe("The bucket name."),
      prefix: z.string().optional().describe("Only return objects whose keys start with this prefix."),
      ...
      startAfter: z.string().optional().describe("Return objects after this key (inclusive pagination)."),
    },
    async (args) => {
      try {
  ...
  server.tool(
    ...
    "List all versions of objects in a versioned B2 bucket, including delete markers.",
    {
      ...
      maxKeys: z.number().int().min(1).max(1000).optional().default(1000),
      keyMarker: z.string().optional().describe("Pagination cursor — key from a previous response."),
      versionIdMarker: z.string().optional().describe("Pagination cursor — version ID from a previous response."),
    },
    async (args) => {
      try {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        const result = await s3.send(new GetObjectAclCommand({
          Bucket: args.bucket,
          Key: args.key,
          VersionId: args.versionId,
        }));
  ...
  server.tool(
    ...
    "Set the canned ACL for a B2 object via the S3-compatible API. Important B2 limitation: objects must have the same ACL as their parent bucket. Setting an object ACL that differs from its bucket will return a 403 Forbidden error. Only 'private' and 'public-read' are supported.",
    {
      ...
      acl: z.enum(["private", "public-read"])
        .describe("The canned ACL to apply. Must match the parent bucket's ACL — B2 does not support object-level ACLs that differ from the bucket."),
      versionId: z.string().optional(),
    },
    async (args) => {
      try {
