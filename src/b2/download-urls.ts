...
export function registerDownloadUrlTools(
  ...
  server.tool(
    ...
    "Generate a time-limited authorization token for downloading files from a private B2 bucket. The token grants access to files whose names start with the given prefix.",
    {
      ...
      fileNamePrefix: z.string().describe("The token grants access to files whose names start with this prefix. Use '' for all files."),
      validDurationInSeconds: z.number().int().min(1).max(604800)
        .describe("How long the token is valid, in seconds. Maximum is 604800 (7 days)."),
      b2ContentDisposition: z.string().optional()
        .describe("Optional Content-Disposition header to use when files are downloaded with this token."),
    ...
    async (args) => {
      try {
        ...
        const optional = ["b2ContentDisposition", "b2ContentLanguage", "b2Expires",
          "b2CacheControl", "b2ContentEncoding", "b2ContentType"] as const;
  ...
  server.tool(
    ...
    "Construct a download URL for a file in a B2 bucket. For private buckets, provide an authorizationToken from b2_get_download_authorization.",
    {
      ...
      authorizationToken: z.string().optional()
        .describe("Authorization token for private bucket access. Get this from b2_get_download_authorization."),
    },
    async (args) => {
      try {
