...
| `b2_create_key`                    | `facade`       | `B2Client.createKey` via `B2Client.createKey`                                                                                                | Sink-backed handler runs when `B2_SECRET_SINK=file` or `inline`; `off` keeps the non-secret unavailable compatibility stub. Cap: `writeKeys`. Pagination: none. Abort: no facade signal in the reviewed SDK. Retry/idempotency: no automatic replay beyond the SDK facade; sink failure returns an MCP error and callers should reconcile/revoke out of band before retrying. Secret: returns durable `applicationKey` once and must go only to the approved sink unless explicit inline mode is set. Error: SDK errors plus sink failures to stable MCP classes.                                                                                |
...
| `b2_create_group_member`           | `partner`      | `PartnerRawClient.createGroupMember` via `B2Client.createGroupMember`                                                                       | Sink-backed handler runs when `B2_SECRET_SINK=file` or `inline`; `off` keeps the non-secret unavailable compatibility stub. Cap: Partner entitlement plus master key. Pagination: none. Abort: SDK Partner transport receives the MCP request signal. Retry/idempotency: non-idempotent create has automatic unauthorized retry disabled; after sink or transport failure, reconcile/eject out of band before retrying. Secret: returned application key must never go to model output in file/off modes. Error: Partner and sink errors to stable MCP classes.                                                              |
...
| `b2_reserve_trial_create_account`  | `partner`      | `PartnerRawClient.reserveTrialCreateAccount` via `B2Client.reserveTrialCreateAccount`                                                       | Sink-backed handler runs when `B2_SECRET_SINK=file` or `inline`; `off` keeps the non-secret unavailable compatibility stub. Cap: Partner/Reserve entitlement plus master key. Pagination: none. Abort: SDK Partner transport receives the MCP request signal. Retry/idempotency: non-idempotent account creation has automatic unauthorized retry disabled; after sink or transport failure, reconcile created accounts out of band before retrying. Secret: returned account credentials must never go to model output in file/off modes. Error: Partner and sink errors to stable MCP classes.                                                                   |
