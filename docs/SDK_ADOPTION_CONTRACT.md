...
Partner Group lifecycle decision (#366): the
[official Partner API introduction](https://www.backblaze.com/apidocs/introduction-to-the-partner-api)
documents account creation, organizing accounts in Groups, listing Groups,
listing Group members, and ejecting Group members, while Group creation remains
documented as a
[web-console setup step](https://www.backblaze.com/docs/cloud-storage-create-a-group-for-the-partner-api)
before `b2_create_group_member` is used. The current `@backblaze-labs/b2-sdk`
0.4 Partner bindings expose the same public runtime set: `listGroups`,
`listGroupMembers`, `createGroupMember`, `ejectGroupMember`, and
`reserveTrialCreateAccount`. There is no documented `b2_create_group`,
`b2_update_group`, or `b2_delete_group` endpoint or SDK binding, so this server
intentionally does not add Group lifecycle tools until a public API/SDK surface
exists.
...
| `s3_get_bucket_lifecycle`          | `s3`           | `/s3` `createS3ClientConfig` plus `B2S3PeerClient.getBucketLifecycle`                                     | Decision #366: add the symmetric S3 lifecycle read instead of relying only on native `b2_list_buckets`, because S3 lifecycle writes are already exposed and B2 documents [`GET ?lifecycle`](https://www.backblaze.com/apidocs/s3-get-lifecycle-configuration). Cap: `readBucketLifecycleRules`. Pagination: none. Abort: peer request must accept abort. Retry/idempotency: read. Secret: none. Error: S3/B2 lifecycle errors to MCP shape; `NoSuchLifecycleConfiguration` normalizes to `configured: false` with no rules. Provider rules without IDs are returned without `id` and require a caller-supplied ID before reuse with `s3_put_bucket_lifecycle`. |
