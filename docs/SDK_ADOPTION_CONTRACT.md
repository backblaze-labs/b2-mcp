...
the time of adoption. The root and customer-hosted workspaces therefore carry a
temporary exact-version `minimumReleaseAgeExclude` entry for
`@backblaze-labs/b2-sdk@0.3.0` so `pnpm install --frozen-lockfile` can verify the
reviewed lockfile. Future SDK bumps must either wait out the release-age window
or add a new reviewed exception; this exception does not cover any other SDK
version.
...
| `b2_create_group_member`           | `partner`      | `PartnerRawClient.createGroupMember` SDK binding reviewed; no `B2Client` production adapter until a durable secret sink exists              | Secret-producing Partner create remains unavailable as a non-secret compatibility stub until secret sink/idempotency work lands, even though the SDK operation exists. Cap: Partner entitlement plus master key. Pagination: none. Abort: SDK Partner transport supports request abort when enabled. Retry/idempotency: requires idempotency key and quarantine/eject recovery for sink failure. Secret: returned application key must never go to model output. Error: Partner and sink errors to stable MCP classes.                                                              |
...
| `b2_reserve_trial_create_account`  | `partner`      | `PartnerRawClient.reserveTrialCreateAccount` SDK binding reviewed; no `B2Client` production adapter until a durable secret sink exists      | Secret-producing trial account create remains unavailable as a non-secret compatibility stub until secret sink/idempotency work lands, even though the SDK operation exists. Cap: Partner/Reserve entitlement plus master key. Pagination: none. Abort: SDK Partner transport supports request abort when enabled. Retry/idempotency: requires idempotency key and quarantine/recovery. Secret: returned account credentials must go only to approved sink. Error: Partner and sink errors to stable MCP classes.                                                                   |
