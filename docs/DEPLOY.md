...
| **Destructive-op gate** | `B2_DESTRUCTIVE_POLICY` — `confirm` (challenge-bound MCP form elicitation on capable clients; otherwise `confirm: true`), `block` (unattended/read-mostly hard refusal without prompting), `allow` (trusted automation; skips elicitation and the confirm/block gate). Set `confirm` explicitly when an HTTP deployment should use client-mediated approval. On HTTP, elicitation accept is client-attested unless your host/proxy enforces consent; only `block` is the built-in hard wall for untrusted clients. | `block`           |
| **Elicitation kill switch** | `B2_DESTRUCTIVE_ELICITATION=false` disables MCP elicitation interception and falls back to the existing `confirm: true` gate under `confirm` policy if a capable client mishandles approval prompts. | enabled           |
