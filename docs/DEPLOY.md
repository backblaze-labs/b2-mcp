...
| **Secret-producing tools** | `b2_create_key`, `b2_create_group_member`, and `b2_reserve_trial_create_account` are registered only as non-secret unavailable compatibility stubs until a reviewed out-of-band secret sink exists.                                         | unavailable       |
| **Tool-result text**       | `B2_MCP_OUTPUT_FORMAT=toon\|json` selects only the LLM-facing `TextContent.text` serialization for structured successes. `structuredContent` and MCP envelopes remain JSON.                                                                 | `toon`            |
