...
| **Destructive-op gate** | `B2_DESTRUCTIVE_POLICY` — `confirm` (MCP elicitation accept on capable clients; otherwise `confirm: true`), `block` (unattended/read-mostly hard refusal), `allow` (trusted; disables only the server-side confirm/block gate).                              | `confirm`         |
