...
| **Destructive-op gate** | `B2_DESTRUCTIVE_POLICY` — `confirm` (challenge-bound MCP form elicitation on capable clients; otherwise `confirm: true`), `block` (unattended/read-mostly hard refusal without prompting), `allow` (trusted automation; skips elicitation and the confirm/block gate). Set `confirm` explicitly when an HTTP deployment should use interactive approval. | `block`           |
