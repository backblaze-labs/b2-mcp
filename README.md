...
| ---------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `B2_DESTRUCTIVE_POLICY`                                          | `stdio: confirm`; HTTP: `block` | Server-side gate on destructive tools: `confirm` uses challenge-bound MCP form elicitation on capable clients and otherwise requires `confirm: true`; `block` refuses directly; `allow` skips the gate and elicitation |
| `B2_DESTRUCTIVE_ELICITATION`                                     | `true`             | Set `false` to disable MCP elicitation interception and fall back to the existing `confirm: true` gate under `confirm` policy |
...
**40 total — 21 native (`b2_*`) + 19 data-plane (`s3_*`).** 34 tools are active in the full surface; 6 native names are unavailable compatibility stubs for stale cached `tools/list` clients or deferred dependencies. The inherited `s3_*` aliases use the AWS S3 SDK against B2's S3-compatible endpoint, with configuration derived from the official B2 SDK `/s3` helper. Buckets, key listing/deletion, Object Lock, notifications, and insights stay native. Under `B2_DESTRUCTIVE_POLICY=confirm` (the stdio default; HTTP defaults to `block` unless set explicitly), fourteen destructive, durable-secret-producing, or protection-weakening tool names request MCP form elicitation when the current request advertises that capability; clients without form elicitation, or with `B2_DESTRUCTIVE_ELICITATION=false`, fall back to `confirm: true`. Accepted elicitation responses are bound to a server-issued challenge covering the tool name, canonical arguments, credential fingerprint, process instance, and expiry, so a response for one destructive call is not accepted for another. On HTTP, an elicitation accept is still client-attested unless your host/proxy enforces consent; only `B2_DESTRUCTIVE_POLICY=block` is the built-in hard wall for untrusted clients. `B2_DESTRUCTIVE_POLICY=block` refuses directly without prompting, and `B2_DESTRUCTIVE_POLICY=allow` proceeds without elicitation for trusted automation. The gated names are the explicit deletes (`s3_delete_object`, `s3_delete_objects`, `s3_abort_multipart_upload`, `b2_delete_bucket`, `b2_delete_key`), PutObject presigning (`s3_get_presigned_url` with `operation: "PutObject"`), Partner group membership changes (`b2_eject_group_member`, `b2_create_group_member`), trial-account reservation (`b2_reserve_trial_create_account`), persistent outbound webhook replacement (`b2_set_bucket_notification_rules`), and the protection-removal or copy/delete policy paths (`b2_update_file_retention` when clearing/bypassing, `b2_update_file_legal_hold` when set off, `b2_update_bucket` when it makes a bucket public or weakens Object Lock/lifecycle/replication, and `s3_put_bucket_lifecycle` when a rule schedules deletion).
