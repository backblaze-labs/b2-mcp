...
- **Control plane (17, native B2 API)** — buckets, application keys, Partner/Groups provisioning, Object Lock, event notifications. *(The S3 API has no equivalent for these.)*
- **Data plane (19, S3-compatible API)** — object upload/download/copy/list/delete, multipart, presigned URLs. *(Forward-compatible; S3 is the standard surface for object data.)*
...
> *"Create a read-only key scoped to my `public-assets` bucket."* · *"Upload `./data.csv` to `reports/may-2026.csv`."* · *"Give me a 1-hour download link for `backups/latest.tar.gz`."* · *"List files under `logs/2026/`."*
