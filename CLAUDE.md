...
- **Destructive-operation gate** (`src/utils/destructive-gate.ts`, all transports). Irreversible/high-impact tools — `s3_delete_object`, `s3_delete_objects`, `s3_abort_multipart_upload`, `b2_delete_bucket`, `b2_delete_key`, `b2_eject_group_member`, and `b2_update_bucket` *only* when it makes a bucket public or disables/clears Object Lock — are gated by `B2_DESTRUCTIVE_POLICY`: `confirm` (default) requires the call to pass `confirm: true` (otherwise it is refused with a description of the effect, and the call never reaches B2); `block` refuses outright (unattended/read-mostly deployments); `allow` disables the gate (trusted single-user). Server-side, so it holds even for MCP clients without the skills layer. Each gated tool exposes an optional `confirm` boolean. `confirm` is defense-in-depth (a fully hijacked model could set it) — pair with `block` or host consent for untrusted contexts.
