...
- Pause and ask for explicit confirmation before `s3_complete_multipart_upload` would complete into an existing destination key in a reused prefix; exclusive write ownership is not overwrite approval.
...
4. For small controlled writes, mint a short-lived PutObject URL with `s3_get_presigned_url` only after the safety gate. For large objects, persist multipart upload ID, part numbers, ETags, and completed-part checkpoints after each part. In a reused prefix, check the destination key and pause for explicit overwrite approval before `s3_complete_multipart_upload`; otherwise restrict multipart completion to the unique run prefix.
...
7. For large backup sets, verify through the external manifest or worker rather than one MCP call per object. Reconcile the destination with paginated `s3_list_objects_v2` pages of at most 1,000 keys, persist verification cursors and sampled row limits, use `s3_head_object` only for bounded samples, changed keys, missing keys, or exception checks, and cap concurrent metadata checks from the worker to the approved rate limit. Retry transient verification failures with bounded exponential backoff and resume interrupted verification from the saved manifest checkpoint before marking the run complete.
