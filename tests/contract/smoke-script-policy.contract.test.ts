...
describe("smoke script release contract", () => {
  ...
  it("keeps the supplemental SDK client smoke advisory and contract-backed", async () => {
    ...
    expect(testingDoc).toContain("@modelcontextprotocol/inspector@2.6.0");
  ...
  it("keeps Inspector smoke locked, sanitized, and supplemental", async () => {
    ...
    expect(parsedPackageJson.devDependencies["@modelcontextprotocol/inspector"]).toBe("2.6.0");
    ...
    expect(lockedInspector?.version).toBe("2.6.0");
    ...
    expect(inspectorSmoke.INSPECTOR_VERSION).toBe("2.6.0");
