...
beforeAll(async () => {
  ...
  if (HAS_S3_CREDS) {
    ...
    if (s3Buckets?.buckets?.length) {
      ...
      const objects = parseResult(await callTool(server, "s3_list_objects_v2", { bucket: firstBucketName, maxKeys: 1 }));
  ...
  const b2Files = firstBucketId
    ? parseResult(await callTool(server, "b2_list_file_names", { bucketId: firstBucketId, maxFileCount: 1 }))
    : null;
...
describe("Protocol layer", () => {
  ...
  it("includes all expected landmark tools", () => {
    ...
    for (const name of [
      "b2_authorize_account", "b2_upload_file", "b2_list_buckets",
      "b2_start_large_file", "b2_update_file_legal_hold",
      "s3_put_object", "s3_get_object", "s3_get_presigned_url",
      "s3_get_object_lock_configuration", "s3_upload_part_copy",
      // Partner API tools
      "b2_list_groups", "b2_create_group_member", "b2_eject_group_member",
      "b2_list_group_members", "b2_reserve_trial_create_account",
...
describe("B2 Bucket tools", () => {
  ...
  liveIt("b2_list_buckets bucketType filter works", async () => {
    const result = parseResult(await callTool(server, "b2_list_buckets", { bucketTypes: ["allPrivate", "allPublic"] }));
    expect(Array.isArray(result.buckets)).toBe(true);
  });
});
...
describe("B2 File tools", () => {
  liveIt("b2_list_file_names returns files from first bucket", async () => {
    if (!firstBucketId) { console.log("  No bucket — skip"); return; }
    const result = parseResult(await callTool(server, "b2_list_file_names", { bucketId: firstBucketId, maxFileCount: 5 }));
  ...
  liveIt("b2_list_file_versions returns version history", async () => {
    ...
    const result = parseResult(await callTool(server, "b2_list_file_versions", { bucketId: firstBucketId, maxFileCount: 5 }));
    expect(result).toHaveProperty("files");
    console.log("  Versions:", result.files.length);
  });

  liveIt("b2_get_file_info returns metadata for a real file", async () => {
    if (!firstFileId) { console.log("  No file ID — skip"); return; }
  ...
  liveIt("b2_get_file_info returns structured error for bad file ID", async () => {
    const result = await callTool(server, "b2_get_file_info", { fileId: "4_bad_id_000000000000000000000000000" });
  ...
  liveIt("b2_get_download_url_for_file returns a URL", async () => {
    if (!firstBucketId) return;
    const files = parseResult(await callTool(server, "b2_list_file_names", { bucketId: firstBucketId, maxFileCount: 1 }));
    if (!files?.files?.length) return;
    const { fileName } = files.files[0];
    const b2Buckets = parseResult(await callTool(server, "b2_list_buckets", {}));
    const bucketName = b2Buckets.buckets[0].bucketName;
    const result = parseResult(await callTool(server, "b2_get_download_url_for_file", { bucketName, fileName }));
  ...
  liveIt("b2_get_download_url_for_file_id returns a URL", async () => {
    ...
    const result = parseResult(await callTool(server, "b2_get_download_url_for_file_id", { fileId: firstFileId }));
    expect(result).toHaveProperty("url");
    expect(result.url).toMatch(/^https:\/\//);
  });
  ...
  liveIt("b2_get_download_authorization returns an auth token", async () => {
    ...
    const result = parseResult(await callTool(server, "b2_get_download_authorization", {
      bucketId: firstBucketId, fileNamePrefix: "", validDurationInSeconds: 3600
    }));
...
describe("B2 Key tools", () => {
  liveIt("b2_list_keys returns application keys", async () => {
    ...
    if (result.keys.length) {
      ...
      console.log("  First key:", k.keyName, "capabilities:", k.capabilities?.slice(0, 3).join(", "));
    }
  });
});
...
describe("B2 Large File tools", () => {
  liveIt("b2_list_unfinished_large_files returns in-progress uploads", async () => {
    if (!firstBucketId) return;
    const result = parseResult(await callTool(server, "b2_list_unfinished_large_files", { bucketId: firstBucketId }));
    expect(result).toHaveProperty("files");
...
describe("S3 Bucket tools", () => {
  ...
  liveS3It("s3_get_bucket_versioning returns versioning status", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_bucket_versioning", { bucket: firstBucketName }));
  ...
  liveS3It("s3_get_bucket_cors returns CORS rules (may be empty)", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_bucket_cors", { bucket: firstBucketName }));
    // Either returns corsRules array or an error (no CORS configured)
    const hasCors = !isError(await callTool(server, "s3_get_bucket_cors", { bucket: firstBucketName }));
    if (hasCors) {
  ...
  liveS3It("s3_get_bucket_lifecycle returns lifecycle rules (may be empty)", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_bucket_lifecycle", { bucket: firstBucketName }));
    const ok = !isError(await callTool(server, "s3_get_bucket_lifecycle", { bucket: firstBucketName }));
    if (ok) {
  ...
  liveS3It("s3_get_bucket_acl returns owner and grants", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_bucket_acl", { bucket: firstBucketName }));
  ...
  liveS3It("s3_get_bucket_location returns region", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_bucket_location", { bucket: firstBucketName }));
  ...
  liveS3It("s3_head_bucket returns error for non-existent bucket", async () => {
    const result = await callTool(server, "s3_head_bucket", { bucket: "this-bucket-does-not-exist-xyz-mcp-test-99999" });
    expect(isError(result)).toBe(true);
    console.log("  Error:", result?.content?.[0]?.text?.slice(0, 100));
  });
});
...
describe("S3 Object tools", () => {
  liveS3It("s3_list_objects_v2 lists objects from first bucket", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_list_objects_v2", { bucket: firstBucketName, maxKeys: 5 }));
  ...
  liveS3It("s3_list_objects (v1) lists objects from first bucket", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_list_objects", { bucket: firstBucketName, maxKeys: 5 }));
    expect(result).toHaveProperty("objects");
  ...
  liveS3It("s3_list_object_versions returns version list", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_list_object_versions", { bucket: firstBucketName, maxKeys: 5 }));
    expect(result).toHaveProperty("versions");
    console.log("  Versions:", result.versions.length, "delete markers:", result.deleteMarkers.length);
  });

  liveS3It("s3_head_object returns metadata for a real object", async () => {
    if (!firstBucketName || !firstObjectKey) { console.log("  No object — skip"); return; }
    const result = parseResult(await callTool(server, "s3_head_object", { bucket: firstBucketName, key: firstObjectKey }));
    expect(result).toHaveProperty("contentLength");
    expect(result).toHaveProperty("contentType");
    console.log("  Object:", firstObjectKey, `(${result.contentLength} bytes, ${result.contentType})`);
  });

  liveS3It("s3_get_object_acl returns object ACL", async () => {
    if (!firstBucketName || !firstObjectKey) return;
    const result = parseResult(await callTool(server, "s3_get_object_acl", { bucket: firstBucketName, key: firstObjectKey }));
    expect(result).toHaveProperty("owner");
  ...
  liveS3It("s3_list_objects_v2 with prefix filter works", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_list_objects_v2", {
      bucket: firstBucketName, prefix: "", delimiter: "/", maxKeys: 10
    }));
    expect(result).toHaveProperty("objects");
...
describe("S3 Presigned URL", () => {
  liveS3It("s3_get_presigned_url generates a GET URL", async () => {
    ...
    const result = parseResult(await callTool(server, "s3_get_presigned_url", {
      bucket: firstBucketName, key: firstObjectKey, method: "GET", expiresIn: 3600
    }));
    expect(result).toHaveProperty("url");
    expect(result.url).toMatch(/^https:\/\//);
  ...
  liveS3It("s3_get_presigned_url generates a PUT URL", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_get_presigned_url", {
      bucket: firstBucketName, key: "mcp-test-presigned-put.txt", method: "PUT", expiresIn: 900
    }));
    expect(result).toHaveProperty("url");
    expect(result.url).toMatch(/^https:\/\//);
...
describe("S3 Multipart tools", () => {
  liveS3It("s3_list_multipart_uploads returns in-progress uploads", async () => {
    if (!firstBucketName) return;
    const result = parseResult(await callTool(server, "s3_list_multipart_uploads", { bucket: firstBucketName }));
...
describe("S3 Object Lock tools", () => {
  liveS3It("s3_get_object_lock_configuration returns config or error", async () => {
    if (!firstBucketName) return;
    const raw = await callTool(server, "s3_get_object_lock_configuration", { bucket: firstBucketName });
...
describe("S3 Write cycle (upload → head → download → delete)", () => {
  ...
  liveS3It("can upload, inspect, and delete a small object", async () => {
    if (!writableBucketName) { console.log("  No writable bucket — skip"); return; }
    console.log("  Using writable bucket:", writableBucketName);

    // Upload
    const putResult = await callTool(server, "s3_put_object", {
      bucket: writableBucketName, key: testKey,
      content: testContent, contentType: "text/plain"
    });
    expect(isError(putResult)).toBe(false);
    console.log("  Uploaded:", testKey);

    // Head
    const headResult = parseResult(await callTool(server, "s3_head_object", { bucket: writableBucketName, key: testKey }));
    ...
    // Download
    const getResult = parseResult(await callTool(server, "s3_get_object", { bucket: writableBucketName, key: testKey }));
    ...
    // Delete
    const delResult = await callTool(server, "s3_delete_object", { bucket: writableBucketName, key: testKey });
  ...
  liveIt("can upload via B2 native API and retrieve file info", async () => {
    if (!writableBucketId) { console.log("  No writable bucket ID — skip"); return; }
    console.log("  Using writable bucket ID:", writableBucketId);

    const fileName = `mcp-b2-test-${Date.now()}.txt`;
    const uploadResult = parseResult(await callTool(server, "b2_upload_file", {
    ...
    // Delete
    const delResult = parseResult(await callTool(server, "b2_delete_file_version", { fileId, fileName }));
...
describe("Partner API — b2_list_groups", () => {
  liveIt("returns groups array or structured unauthorized error", async () => {
    ...
    if (!isError(result)) {
      ...
      console.log("  Not a Partner API account (expected):", result?.content?.[0]?.text?.slice(0, 80));
...
describe("Error handling", () => {
  ...
  liveS3It("s3_head_bucket: structured error for non-existent bucket", async () => {
    const result = await callTool(server, "s3_head_bucket", { bucket: "this-bucket-does-not-exist-xyz-99999" });
    expect(isError(result)).toBe(true);
  });
  ...
  liveS3It("s3_get_object: structured error for non-existent key", async () => {
    ...
    const result = await callTool(server, "s3_get_object", {
      bucket: firstBucketName, key: "this-key-definitely-does-not-exist-mcp-test.xyz"
    });
    expect(isError(result)).toBe(true);
