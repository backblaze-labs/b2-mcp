...
describe("Tool inventory", () => {
  ...
  it("all tools use b2_ / bz_ / s3_ prefix", () => {
    const invalid = toolNames.filter(n => !n.startsWith("b2_") && !n.startsWith("bz_") && !n.startsWith("s3_"));
...
describe("Every tool has a valid description", () => {
  const toolList = [
    // B2 native
    "b2_authorize_account", "b2_list_buckets", "b2_create_bucket", "b2_delete_bucket",
    "b2_update_bucket", "b2_get_bucket_notification_rules", "b2_set_bucket_notification_rules",
    "b2_upload_file", "b2_download_file_by_id", "b2_download_file_by_name",
    "b2_list_file_names", "b2_list_file_versions", "b2_get_file_info",
    "b2_delete_file_version", "b2_hide_file", "b2_copy_file", "b2_get_upload_url",
    "b2_start_large_file", "b2_get_upload_part_url", "b2_upload_part",
    "b2_copy_part", "b2_finish_large_file", "b2_cancel_large_file",
    "b2_list_parts", "b2_list_unfinished_large_files",
    "b2_get_download_authorization", "b2_get_download_url_for_file", "b2_get_download_url_for_file_id",
    "b2_create_key", "b2_list_keys", "b2_delete_key",
    ...
    // S3-compatible
    "s3_list_buckets", "s3_create_bucket", "s3_delete_bucket", "s3_head_bucket",
    "s3_get_bucket_versioning", "s3_put_bucket_versioning",
    "s3_get_bucket_cors", "s3_put_bucket_cors", "s3_delete_bucket_cors",
    "s3_get_bucket_lifecycle", "s3_put_bucket_lifecycle",
    "s3_get_bucket_acl", "s3_put_bucket_acl",
    "s3_put_object", "s3_get_object", "s3_delete_object", "s3_delete_objects",
    "s3_head_object", "s3_copy_object",
    "s3_list_objects_v2", "s3_list_object_versions", "s3_get_object_acl", "s3_put_object_acl",
    "s3_create_multipart_upload", "s3_upload_part", "s3_complete_multipart_upload",
    "s3_abort_multipart_upload", "s3_list_multipart_uploads", "s3_list_parts",
    "s3_get_presigned_url",
    "s3_get_object_lock_configuration", "s3_put_object_lock_configuration",
    "s3_get_object_legal_hold", "s3_put_object_legal_hold",
    "s3_get_object_retention", "s3_put_object_retention",
    "s3_get_bucket_location",
    "s3_get_bucket_encryption", "s3_put_bucket_encryption", "s3_delete_bucket_encryption",
    "s3_get_bucket_logging", "s3_put_bucket_logging",
    "s3_delete_bucket_lifecycle", "s3_list_objects", "s3_upload_part_copy",
  ];
...
describe("Tools that need a bucket param declare it as required", () => {
  const bucketTools = [
    "s3_create_bucket", "s3_delete_bucket", "s3_head_bucket",
    "s3_get_bucket_versioning", "s3_put_bucket_versioning",
    "s3_get_bucket_cors", "s3_put_bucket_cors", "s3_delete_bucket_cors",
    "s3_get_bucket_lifecycle", "s3_put_bucket_lifecycle",
    "s3_get_bucket_acl", "s3_put_bucket_acl",
    "s3_list_objects_v2", "s3_list_objects", "s3_list_object_versions",
    "s3_get_bucket_location", "s3_get_bucket_encryption",
    "s3_put_bucket_encryption", "s3_delete_bucket_encryption",
    "s3_get_bucket_logging", "s3_put_bucket_logging",
...
describe("Object tools require both bucket and key", () => {
  ...
  const objectTools = [
    "s3_put_object", "s3_get_object", "s3_delete_object",
    "s3_head_object",
...
describe("Multipart tools require uploadId where needed", () => {
  const uploadIdTools = [
    "s3_upload_part", "s3_complete_multipart_upload",
    "s3_abort_multipart_upload", "s3_list_parts",
    "s3_upload_part_copy",
  ];
...
describe("Tool descriptions mention B2 or S3 context", () => {
  const contextTerms = ["b2", "s3", "bucket", "object", "file", "key", "upload",
                        "download", "backblaze", "multipart", "version", "lock",
                        "presigned", "cors", "lifecycle", "acl", "encryption", "logging",
                        "group", "backup", "account", "computer", "partner"];
