...
function expectedSkillPackFiles(): string[] {
  const manifest = readJson<SkillsManifest>("skills/manifest.json");
  return ["skills/manifest.json", ...manifest.skills.map((skill) => skill.path)].sort();
}
...
describe("package surface policy", () => {
  ...
  it("keeps repo-only policy files out of the published npm package", () => {
    ...
    expect(pkg.files).toContain("skills/manifest.json");
  ...
  it("does not pack non-SKILL files from skill directories", () => {
    const extraSkillFile = join(root, "skills", "backup-restore", "notes.tmp");
    writeFileSync(extraSkillFile, "temporary packlist fixture\n");
    ...
    try {
      ...
      expect(files).not.toContain("skills/backup-restore/notes.tmp");
      expect(files.filter((file) => file.startsWith("skills/")).sort()).toEqual(
        expectedSkillPackFiles(),
      );
    } finally {
