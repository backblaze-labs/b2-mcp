...
const UNSUPPORTED_MARKDOWN_SAMPLES: ReadonlyArray<readonly [string, string]> = [
  ...
  ["code span with stripped boundary spaces", "run `  spaced  ` here"],
  ["code span with single boundary spaces", "run ` spaced ` here"],
...
const SUPPORTED_MARKDOWN_SAMPLES: ReadonlyArray<readonly [string, string]> = [
  ...
  ["code span with internal spaces", "the `a  b` token"],
  ["code span with leading-only space", "the ` lead` token"],
  ["code span with trailing-only space", "the `trail ` token"],
