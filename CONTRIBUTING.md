...
| --- | --- | --- |
| Typecheck | `pnpm run typecheck` | Source and tests compile against the conservative `@types/node` 22.20.2 baseline (below the 22.22.2 engine floor; see the note near the end of this guide). |
...
`@types/node` is pinned to a conservative 22.20.2 baseline (below the 22.22.2 engine floor; @types/node has no 22.22.x release) so TypeScript does not allow
